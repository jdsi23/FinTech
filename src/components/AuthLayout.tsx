...
export function FormField({
  ...
  prefix,
  ...props
}: { label: string; prefix?: ReactNode } & React.InputHTMLAttributes<HTMLInputElement>) {
  const inputClass =
    'w-full rounded-md border border-slate-300 py-2 text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100'
  return (
    <label className="mb-3 block text-sm">
      <span className="mb-1 block font-medium text-slate-700 dark:text-slate-300">{label}</span>
      {prefix ? (
        <div className="relative">
          <span className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-500 dark:text-slate-400">
            {prefix}
          </span>
          <input {...props} className={`${inputClass} pl-7 pr-3`} />
        </div>
      ) : (
        <input {...props} className={`${inputClass} px-3`} />
      )}
    </label>
  )
}
